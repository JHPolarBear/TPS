...
void ATPSCharacter::Tick(float DeltaTime)
{
	...
	if (GetIsFiring() && GetCurrentBulletNum() > 0)
	{
		if (FireDeltaTime >= Weapon->GetFireRate())
		{
			...
			if (!Weapon->bFullAutoFire || GetCurrentBulletNum() == 0)
			{
				SetIsFiring(false);
			}
		}
...
void ATPSCharacter::OnReloadingStart()
{
	...
	if(Weapon->GetIsReloading())
	{
		...
		TPSAnimInstance->SetIsReloading(false);
	...
	else
	{
		...
		TPSAnimInstance->SetIsReloading(true);
...
int ATPSCharacter::GetCurrentBulletNum()
{
	if (Weapon != NULL)
		return Weapon->GetCurrentBulletNum();
	else
		return 0;
}
